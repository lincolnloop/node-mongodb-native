...
MongoClient.connect = function(url, options, callback) {
  ...
  console.log('[MC] servers length', object.servers.length)
  console.log('[MC]', object.servers)
  for(var i = 0; i < object.servers.length; i++) {
    ...
    console.log('[MC] domain socket', object.servers[i].domain_socket)
    ...
    var connectFunction = function(__server) {
      ...
      new Db(object.dbName, __server, {w:1, native_parser:false}).open(function(err, db) {
        ...
        console.log('[MC] totalNumberOfServers', totalNumberOfServers)
        ...
        if(!err) {
          ...
          console.log('[MC] adding to errorServers', __server, err)
          error = err;
        ...
        if(totalNumberOfServers == 0) {
          ...
          console.log('---------------')
          console.log('totalNumberOfMongosServers', totalNumberOfMongosServers)
          console.log('totalNumberOfMongodServers', totalNumberOfMongodServers)
          console.log('rs_options', object.rs_options)
          console.log('==============')
          ...
          if(totalNumberOfMongodServers == 0
            ...
            console.log('[MC] domain socket connectFunction', obj.domain_socket)
            ...
            console.log('[MC] finalServers', finalServers)
            ...
            if(totalNumberOfMongodServers > 0) {
              try {
                ...
                console.log('[MC] got err', err)
                return callback(err, null);
              }
            } else {
              console.log('[MC] Mongos options', object.mongos_options)
              serverConfig = new Mongos(finalServers, object.mongos_options);
            }
          }

          if(serverConfig == null) {
            console.log('[MC] serverConfig null')
            return process.nextTick(function() {
              try {
